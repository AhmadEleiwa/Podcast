...
const links = [
    ...
    ["Company", "Brand", "FAQ", "Tutorials", "About Us", "We're Hirding!", "Support"],
    ...
    flexWrap={'wrap'}  >
        {links.map((item, i) => {
            ...
            flexDirection={'column'}>
                {item.map((text, j) => {
                    return <Link to='' key={Math.random().toString(25)} active={j === 0} style={{ marginBottom: j === 0 ? '1em' : '0.8em' }} >{text}</Link>
                })}
            </Box>
        })}
        <Box display={'flex'} position={'absolute'} gap='2em' right={{xs:'45%', lg:'-2em'}} bottom={'20%'} >
